...
```{r clean_up}
sample_data <- separate(sample_data, title, into = c("nothing", "status"), sep = "Muscle PCOS ")
sample_data <- separate(sample_data, status, into = c("nothing", "status"), sep = "after ")
sample_data <- separate(sample_data, status, into = c("status", "id"), sep = " ")
sample_data$status[sample_data$status=="pioglitazone"] <- "after_pioglitazone"
sample_data <- separate(sample_data, nothing, into = c("status_2", "id_2"), sep = " ")

sample_data$id <- ifelse(is.na(sample_data$id), sample_data$id_2, sample_data$id)
sample_data$status <- ifelse(is.na(sample_data$status), sample_data$status_2, sample_data$status)
```
...
Here I am going to get the second comparison of fit2 (Young-old) and get the top genes
```{r}
