...
if (length(gse) > 1) idx <- grep("GPL23159	", attr(gse, "names")) else idx <- 1
gset1 <- gse[[idx]]
...
All samples are young, its checking diabetes and control
```{r}
sample_data$age = sample_data$`exercise status:ch1`
```
...
map_age <- function(age_category) {
  ...
  age_vector <- ifelse(age_category == "pre_exercise",  0,1)
