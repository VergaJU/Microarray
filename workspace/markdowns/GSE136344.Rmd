...
if (length(gse) > 1) idx <- grep("GPL5175", attr(gse, "names")) else idx <- 1
gset1 <- gse[[idx]]
...
Here they are using old, young and old with Metabolic syndrome, but since it is a common syndrome, I will leave it, in case I wanted to remove it the line will be:
```{r}
#sample_data <- sample_data[sample_data$	characteristics_ch1.2 != 'disease state: metabolic syndrome', ]
...
symbol_column  <- gene_table$GB_LIST
...
dds_df = subset(top, select=c("GB_LIST","logFC","adj.P.Val","P.Value", "ID"))
...
tT <- subset(up_reg_epi, select=c("GB_LIST","logFC","adj.P.Val","P.Value","ID"))
