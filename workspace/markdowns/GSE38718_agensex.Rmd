...
map_age <- function(age_category) {
  age_vector <- ifelse(age_category == "Young_Women", 0, ifelse(age_category == "Young_Men", 1, ifelse(age_category == "Old_Women", 2, 3)))
...
gsms <- map_age(sample_data$age_sex)
...
gs <- factor(sml)
groups <- make.names(c("Young_Women", "Young_Men" , "Old_Women" ,  "Old_Men"))
levels(gs) <- groups
gset$group <- gs
```
...
```{r}
gset$group <- sample_data$age_sex
```
...
col.status <- c("pink","cyan","red", "blue")[batches]
...
c1 = "Young"
c2 = "Old"
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_sex.csv", sep = "")
...
cts <- paste(groups, c(tail(groups, -1),tail(groups, -2), tail(groups, -3), head(groups, 1)), sep="-")
...
Let's see the heatmap before the selection
...
ann_colors <- list(
  Subtype = c(Young_Women = "pink", Old_Women = "red", Young_Men = "cyan", Old_Men = "blue"))
...
```{r}
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_Young_Women-Young_Men.csv", sep = "")
ct <- 1
...
## Young_Men-Old_Women
```{r}
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_Young_Men-Old_Women.csv", sep = "")
ct <- 2
de_a(doc_name=doc_name, ct=ct)
#Create the top table for the comparison (coef) we want
...
## Old_Women-Old_Men
```{r}
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_Old_Women-Old_Men.csv", sep = "")
ct <- 3
de_a(doc_name=doc_name, ct=ct)
#Create the top table for the comparison (coef) we want
...
Old_Men-Old_Women
```{r}
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_Old_Men-Young_Women.csv", sep = "")
ct <- 4
de_a(doc_name=doc_name, ct=ct)
#Create the top table for the comparison (coef) we want
...
Young_Woman-Old_Men
```{r}
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_Young_Men-Old_Men.csv", sep = "")
ct <- 5
de_a(doc_name=doc_name, ct=ct)
#Create the top table for the comparison (coef) we want
...
Old women Youg women
```{r}
doc_name <- paste("DEG_list_", my_id, "_", c1, "_vs_", c2, "_Young_men-Old_man.csv", sep = "")
ct <- 7
de_a(doc_name=doc_name, ct=ct)
#Create the top table for the comparison (coef) we want
...
col.status <- c("blue","red","dark green", "black")[batches]
plotMDS(gset, col = col.status, pch = 16)
```
