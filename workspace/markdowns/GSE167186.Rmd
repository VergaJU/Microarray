...
if (length(gse) > 1) idx <- grep("GPL20301", attr(gse, "names")) else idx <- 1
gset1 <- gse[[idx]]
...
unique(sample_data$`age:ch1`)
```
They already separete them into healthy old and sarcopenic
```{r see_unique_categories}
...
map_age <- function(age_category) {
  ...
  #age_vector <- ifelse(age_category == "Young", 0, ifelse(age_category == "Older", 2, 1))
...
gs <- factor(sml)
groups <- make.names(c("Old","Middle-age", "Young"))
levels(gs) <- groups
gset$group <- gs
```
