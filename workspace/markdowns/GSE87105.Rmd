...
In this case, age:ch1 has the class directly on the groups
...
gsms <- map_age(sample_data$`age:ch1`)
...
gs <- factor(sml)
groups <- make.names(c("Young", "Middle.aged","Old"))
levels(gs) <- groups
gset$group <- gs
```
...
```
```{r Experiment}
# try to keep only those 661 genes that were marked as true
```
...
symbol_column  <- gene_table$ILMN_Gene
...
dds_df = subset(top, select=c("Symbol","logFC","adj.P.Val","P.Value", "ID"))
...
tT <- subset(up_reg_epi, select=c("Symbol","logFC","adj.P.Val","P.Value","ID"))
