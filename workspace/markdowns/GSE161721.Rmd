...
if (length(gse) > 1) idx <- grep("GPL7144", attr(gse, "names")) else idx <- 1
gset2 <- gse[[idx]]
...
```{r clean_up}
sample_data <- separate(sample_data, title, into = c("FH", "sample"), sep = "_")
sample_data 
```
...
metadata_doc_name <- paste("samples_", my_id, "A.csv", sep = "")
expression_doc_name <- paste("expression_", my_id, "A.csv", sep = "")
...
symbol_column  <- gene_table$ENTREZ_GENE_ID
symbol_df <- data.frame(entrez_id = symbol_column)
