...
Getting the samples from platform GPL96 from the experiment (in the bioproject)
...
if (length(gse) > 1) idx <- grep("GPL97", attr(gse, "names")) else idx <- 1
gset2 <- gse[[idx]]
...
# Combine the GeoSets row-wise
combined_geosets <- cbind(gset1, gset2)
```
...
unique(sample_data$source_name_ch1)
```

In this case, source_name_ch1 has the class in a compose string.  I will separate them by numeric age

```{r clean_up}
sample_data <- separate(sample_data, source_name_ch1, into = c("no","age"), sep = "vastus lateralis biopsy ")
sample_data <- separate(sample_data, age, into = c("age", "no"), sep = " ")
...
features
```
Here, I will need a function to take a column and gives me a vector, 0 is it is less than 35 yo, 1 if it is less than 65 age and 2 if it is 65 or more
```{r age_category_function}
map_age <- function(age_category) {
  age_vector <- ifelse(age_category <35, 0, ifelse(age_category <65, 1, 2))
...
c1 = "Young"
c2 = "Old"
doc_name <- paste("workspace/DEG_list_", my_id, "_", c1, "_vs_", c2, ".csv", sep = "")
...
Here I am going to get the first comparison of fit2 (Young-old) and get the top genes
```{r}

#Create the top table for the comparison (coef) we want
top <- topTable(fit2, coef=1, adjust.method = "fdr", sort.by="B", number = Inf)
nrow(top)
```
...
We will use now the second one
```{r eval = FALSE}
...
We use it, we will get the mapped data
...
c1 = "Young"
c2 = "Old"
doc_name <- paste("workspace/DEG_list_", my_id, "_", c1, "_vs_", c2, "_probeB.csv", sep = "")
EXPECTED_PROPORTION = 0.01
```
